...
import { selectBadgeDefinitions, selectAwardsForStudent, fetchBadgeDefinitions, fetchAwardsForStudent } from '@/store/slices/badgesSlice';
...
export default function BadgeDisplay({ studentId, showDetails = false, limit }: BadgeDisplayProps) {
  ...
  const awardsAll = useAppSelector((state) => selectAwardsForStudent(state, studentId));
  const userBadges = (limit ? awardsAll.slice(0, limit) : awardsAll);
  ...
  React.useEffect(() => {
    if (studentId) {
      dispatch(fetchAwardsForStudent({ studentId }));
    }
  }, [dispatch, studentId]);
      ...
      {limit && userBadges.length >= limit && (
        <div className="text-center">
          <button className="text-blue-600 hover:text-blue-700 text-sm font-medium">
            View All Badges ({awardsAll.length})
          </button>
        </div>
      )}
    </div>
