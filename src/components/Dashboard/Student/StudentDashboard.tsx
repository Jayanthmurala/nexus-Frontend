    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Pending */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-2">Pending</h3>
            {pendingApplications.length > 0 ? (
              <div className="space-y-2">
                {pendingApplications.map((application) => {
                  ...
                  if (!project) return null;
                  return (
                    <div key={application.id} className="p-3 bg-gray-50 border border-gray-200 rounded-lg flex items-center justify-between">
                      <div>
                        <div className="font-medium text-gray-900 text-sm line-clamp-1">{project.title}</div>
                        <div className="text-xs text-gray-500">Applied on {new Date(application.appliedAt).toLocaleDateString()}</div>
                      </div>
                      <span className="ml-3 px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">Pending</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Accepted */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-2">Accepted</h3>
                ...
                {acceptedApplications.map((application) => {
                  ...
                  return (
                    <div key={application.id} className="p-3 bg-gray-50 border border-gray-200 rounded-lg flex items-center justify-between">
                      ...
                      <span className="ml-3 px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">Accepted</span>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div className="text-sm text-gray-500">No accepted applications</div>
            )}
          </div>

          {/* Rejected */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-2">Rejected</h3>
                ...
                {rejectedApplications.map((application) => {
                  ...
                  return (
                    <div key={application.id} className="p-3 bg-gray-50 border border-gray-200 rounded-lg flex items-center justify-between">
                      ...
                      <span className="ml-3 px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">Rejected</span>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div className="text-sm text-gray-500">No rejected applications</div>
            )}
          </div>
        </div>
      </div>
