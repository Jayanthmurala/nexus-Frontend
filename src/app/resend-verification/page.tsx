...
export default function ResendVerificationPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await http.post("/v1/auth/resend-verification", { email });
      setMessage(data?.message || "If the email exists, a verification link has been sent.");
        ...
        <div className="rounded-2xl border bg-white dark:bg-slate-950 backdrop-blur shadow-sm p-6 md:p-8">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Enter your account email to receive a new verification link.</p>
            </div>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            >
              {loading ? (<><Loader2 className="h-5 w-5 animate-spin" /> Sending...</>) : (<>Send verification</>)}
            </button>
          </form>

          {debugUrl && (
            <p className="mt-4 text-xs text-slate-500">
              Dev only: <a href={debugUrl} className="text-blue-600 hover:underline">Open verification link</a>
            </p>
          )}
