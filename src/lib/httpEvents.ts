...
const EVENTS_API_BASE = process.env.NEXT_PUBLIC_EVENTS_API_BASE_URL || "http://localhost:4004";
...
httpEvents.interceptors.request.use(async (config: InternalAxiosRequestConfig) => {
  try {
    ...
    if (memToken && !(config.headers as any)?.Authorization) {
      ...
      try { if (process.env.NODE_ENV !== 'production') console.debug('[httpEvents] Authorization attached (memToken)'); } catch {}
      return config;
    }
    ...
    if (accessToken && !(config.headers as any)?.Authorization) {
      ...
      try { if (process.env.NODE_ENV !== 'production') console.debug('[httpEvents] Authorization attached (session)'); } catch {}
    }
  } catch {}
  try { if (process.env.NODE_ENV !== 'production') console.debug('[httpEvents] Proceeding request', { hasAuth: !!(config.headers as any)?.Authorization, url: config.url }); } catch {}
  return config;
});

// Handle 401 (and 500 with expired JWT) by attempting refresh against auth-service and retrying once
httpEvents.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (shouldRefresh && !original?._retry) {
      ...
      try { if (process.env.NODE_ENV !== 'production') console.warn('[httpEvents] auth error received, attempting token refresh', { status, code: data?.code }); } catch {}
      const refreshed = await refreshToken(AUTH_API_BASE);
      if (refreshed && refreshed.accessToken) {
        original.headers = original.headers || {};
        original.headers.Authorization = `Bearer ${refreshed.accessToken}`;
        try { if (process.env.NODE_ENV !== 'production') console.debug('[httpEvents] Retrying request after refresh', { url: original.url }); } catch {}
        return httpEvents(original);
      }
